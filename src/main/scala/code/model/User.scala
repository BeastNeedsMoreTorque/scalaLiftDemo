    ...
    tryo {
      DB.use(DefaultConnectionIdentifier) { connection =>
        ...
        val updatedCount = Product.getProductByLcboId(p.lcboId).fold {
          ...
          val userProd = userProducts.where(u => u.userid === id.get and u.productid === q.id).forUpdate.headOption
          userProd.fold {
            // (Product would be stored in DB with no previous user interest)
            UserProduct.createRecord.userid(id.get).productid(q.id).selectionscount(quantity).save // cascade save dependency using Active Record pattern.
            quantity.toLong
          } { up =>
