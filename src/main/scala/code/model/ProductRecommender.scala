...
trait ShufflingProductRecommender extends ProductRecommender {

  def UseRandomSeed: Boolean
  def FixedRNGSeed: Int
  ...
  def getSerialResult(theStore: IStore,
    ...
    val prods = fetcher.fetchByStoreCategory(theStore.lcboId, category, Store.MaxSampleSize) // take a hit of one go to LCBO, querying by category, no more.
  ...
  /**
    ...
    */
  override def recommend(theStore: IStore,
                         category: String,
                         requestSize: Int,
                         fetcher: ProductFetcher): Box[Iterable[(IProduct, Long)]] = {
