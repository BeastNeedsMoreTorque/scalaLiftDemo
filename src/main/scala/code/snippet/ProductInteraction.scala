...
class ProductInteraction extends Loggable {
  ...
  def render = {
    ...
    def prodDisplayJS(prod: Product) =  SetHtml("productImg", <img src={prod.imageThumbUrl}/>)
    val hideProdDisplayJS =  JsHideId("prodDisplay")
    val showProdDisplayJS = JsShowId("prodDisplay")
    ...
    def recommend(): JsCmd = {
      ...
      product match {
        ...
        case _ =>
          maySelect() & toggleButtonsToConsumeJS & showProdDisplayJS & setConfirmJS  // normal processing  (ProductConsume does it the other way around as it plays opposite role as to when it should be active)
      }
    }
    ...
    def consume(): JsCmd = {
      ...
      product match {
        case Full(p)  =>
          mayConsume(p) & toggleButtonsToRecommendJS & hideProdDisplayJS & setConfirmJS  // we got notified that we have a product that can be consumed and user expressed interest in consuming. So, try it as a simulation by doing a DB store.
    ...
    val addCancelImgElem: CssSel = "@cancel *+" #> <img src="/images/cancel.png" alt="cancel represented as X"/>  // since above completely wiped out the XML (HTML5) node, we just lost the nice image with button, so add it back by adding a DOM element with img underneath the button.
